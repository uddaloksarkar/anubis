...
def sample_from_model(texts, min_words=1, prompt_tokens=30, prompt=''):
    # prompt_tokens = len(base_tokenizer.encode(prompt)) # anubis: if we want to use the prompt then results are bad
...
def get_roc_metrics(real_preds, sample_preds):
    # anubis: added average real preds
...
def get_perturbation_results(span_length=10, n_perturbations=1, n_samples=500):
    ...
    p_sampled_text = copy.deepcopy(p_original_text) # anubis: proxy, we dont need sampled text
    ...
    for res in tqdm.tqdm(results, desc="Computing log likelihoods"):
        ...
        p_sampled_ll = copy.deepcopy(p_original_ll) # anubis: proxy, we dont need sampled text
...
def generate_samples(raw_data, batch_size, prompt=''):
    ...
    for batch in range(len(raw_data) // batch_size):
        ...
        sampled_text = copy.deepcopy(original_text) # anubis : we really don't need sample from model here - this is to increase the speed
...
def generate_data(dataset, key, whichtask): # anubis: added whichtask
    # anubis: hardcoding the load dataset
    '''
    # load data
    ...
    #anubis: the  following jeopardises the samples
    '''
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task', type=str, default="4")    # anubis: added task
    ...
    data = generate_data(args.dataset, args.dataset_key, whichtask=args.task) # anubis: added task
    ...
    if not args.baselines_only:
        ...
        for n_perturbations in n_perturbation_list:
            ...
            for perturbation_mode in ['z']: #anubis : removing mode 'd'
